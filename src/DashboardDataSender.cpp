 ...
 */
void DashboardDataSender::sendVisionData(double targetX, vector<Target> targets) {
...
void DashboardDataSender::sendVisionData(double targetX, vector<Ball> targets) {
	if (visionTimer->Get() < 0.1)
		return;
	visionTimer->Reset();
	Dashboard &dash = DriverStation::GetInstance()->GetHighPriorityDashboardPacker();
 ...
 */
void DashboardDataSender::sendIOPortData(bool button4, bool button5, bool button2, bool ballSwitchTriggered) {
	if (IOTimer->Get() < 0.1)
		return;
	IOTimer->Reset();
...
//					dash.AddFloat((float) AnalogModule::GetInstance(1)->GetAverageVoltage(i));
				...
				//}
				dash.AddFloat(button4);
				dash.AddFloat(button5);
				dash.AddFloat(button2);
				dash.AddFloat(0);
				dash.AddFloat(0);
				dash.AddFloat(0);
				dash.AddFloat(DriverStation::GetInstance()->GetBatteryVoltage());
				dash.AddFloat((float)ballSwitchTriggered);
			}
			dash.FinalizeCluster();
			dash.AddCluster();
			{
